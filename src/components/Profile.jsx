...
export const Profile = ({ character }) => {
  ...
  useEffect(() => {
    fetchSingleSpecies(character).then((output) => setSpecies(output))

    fetchFilms().then((output) => setAllFilms(output))

    fetchStarships().then((output) => setStarships(output))
  }, [])

  let matchPersonToFilm = matchPersonToFilms(allFilms, parsedSubjectUrls(character.films)).length
    ? matchPersonToFilms(allFilms, parsedSubjectUrls(character.films))
    : ['films not present']

  let matchPersonToStarship = matchPersonToStarships(
    starships,
    parsedSubjectUrls(character.starships),
  ).length
    ...
    <>
      ...
      <div className="d-flex">
              ...
              <div key={index} className="card p-2">
                <div className="card-body p-2 mx-auto">
                  <p className="card-text">
                    Episode {film.episode}: {film.title || 'not available'}
                  </p>
                </div>
              </div>
            )
        ...
        <div className="w-50 pt-5 p-2 mx-auto">
          <div className="card">
            <div className="card-body mx-auto">
              <h5 className="card-title">Starships Flown:</h5>
            </div>
          </div>
